...
namespace MagicWarlock.Classes.Other
{
    ...
    //hela denna ät stock, inget ska vara ändrat
    ...
    class ExplosionManager
    {
        ...
        public void AddExplosion(Vector2 location, Vector2 momentum)
        {
            ...
            {
                ExplosionParticles.Add(new Particles(pieceLocation, texture, pieceRectangles[rand.Next(0, pieceRectangles.Count)], randomDirection(pieceSpeedScale) + momentum, Vector2.Zero, explosionMaxSpeed, durationCount, initialColor, finalColor));
            }

            int points = rand.Next(minPointCount, maxPointCount + 1);
            for (int x = 0; x < points; x++)
            {
                ExplosionParticles.Add(new Particles(location, texture, pointRectangle, randomDirection(rand.Next(pointSpeedMin,pointSpeedMax)) + momentum, Vector2.Zero, explosionMaxSpeed, durationCount, initialColor, finalColor));
            }
